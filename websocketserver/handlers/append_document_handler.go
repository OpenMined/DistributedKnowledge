...
func HandleAppendDocument(authService *auth.Service, wsServer *ws.Server) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		var payload AppendDocumentPayload
		...
		if err != nil {
			securityLogger.LogAuthEvent(auth.SecurityEvent{
				...
				IP:        clientIP,
				Success:   false,
				Details:   fmt.Sprintf("Error creating document append message: %v", err),
			})
			auth.SendAuthErrorResponse(w, "Error creating document append message", http.StatusInternalServerError)
			return
		}
		...
		log.Printf("Processing document append operation from %s for file: %s", fromUserID, payload.Filename)
		securityLogger.LogAuthEvent(auth.SecurityEvent{
			Timestamp: time.Now(),
			Event:     auth.EventDirectMessageSending,
			UserID:    fromUserID,
			IP:        clientIP,
			Success:   true,
			Details:   fmt.Sprintf("Sending document append message to recipient: %s", recipientID),
		})
		...
		securityLogger.LogAuthEvent(auth.SecurityEvent{
			...
			IP:        clientIP,
			Success:   true,
			Details:   fmt.Sprintf("Successfully appended to document %s and received response", payload.Filename),
		})
