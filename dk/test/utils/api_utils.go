...
// RunAPIMigrations creates all API management tables in the given database
...
// VerifyAPITables checks if expected API management tables exist in the database
func VerifyAPITables(db *sql.DB) []string {
	...
	for _, tableName := range expectedTables {
		...
		if err != nil || name != tableName {
			failures = append(failures, tableName)
		}
	}
