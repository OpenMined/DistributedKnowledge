...
import { formatMessagesForProvider, setupMCPConfig, processLLMResponse, processToolCalls } from '../utils'
...
export class OpenAIProvider implements LLMProviderInterface {
  ...
  async streamMessage(
    ...
    try {
      ...
      console.log(`[${conversationId}] Initial response received with ${response.toolCalls.length} tool calls`);
  ...
  private async makeStreamingRequest(params: {
    ...
    console.log(`[${conversationId}] Making request with ${useTools ? 'tools' : 'no tools'}, tool_choice: ${toolChoice}`);
          ...
          if (line.startsWith('data: ')) {
            try {
              ...
              try {
                ...
                const errorPosition = parseInt(jsonError.message.match(/position (\d+)/)?.[1] || '-1');
                ...
                console.log(`[${conversationId}] Error position: ${errorPosition}, line: ${errorLine}, column: ${errorColumn}`);
                ...
                try {
                  ...
                  if (jsonError.message.includes("Expected ',' or '}'")) {
                    ...
                    if (errorPosition > 0 && errorPosition < jsonStr.length) {
                      ...
                      fixed = jsonStr.substring(0, errorPosition) + "}" + jsonStr.substring(errorPosition);
                      console.log(`[${conversationId}] Attempting to fix missing comma/brace at position ${errorPosition}`);
                    }
                    ...
                    if (openBraces > closeBraces) {
                      ...
                      console.log(`[${conversationId}] Adding ${openBraces - closeBraces} missing closing braces`);
                    }
                    if (openBrackets > closeBrackets) {
                      fixed = fixed + ']'.repeat(openBrackets - closeBrackets);
                      console.log(`[${conversationId}] Adding ${openBrackets - closeBrackets} missing closing brackets`);
                    }
                    ...
                    const singleQuotesFixed = fixed.replace(/'([^']*)'(?=\s*:)/g, '"$1"').replace(/:\s*'([^']*)'/g, ': "$1"');
                    ...
                    if (fixed.includes('{')) {
                      ...
                      if (match && match[0]) {
                        try {
                          ...
                          console.log(`[${conversationId}] Extracted partial valid JSON object: ${match[0]}`);
                        } catch (extractError) {
                  ...
                  console.error(`[${conversationId}] Unable to fix malformed JSON, skipping chunk: ${fixError.message}`);
    ...
    console.log(`[${conversationId}] Stream complete: content length=${responseContent.length}, tool calls=${validatedToolCalls.length}`);
   ...
   * @private
   */
  private processToolCallChunk(toolCallDeltas: any[], toolCalls: any[], conversationId: string): void {
    try {
      ...
      if (!toolCalls.length) {
        for (const tc of toolCallDeltas) {
          ...
          if (!tc.function) {
            console.warn(`[${conversationId}] Tool call delta missing function property: ${JSON.stringify(tc)}`);
            continue;
          }

          // Create tool call with safe defaults
          console.log(`[${conversationId}] Tool call function arguments:`, JSON.stringify(tc.function?.arguments));
          ...
          console.log(`[${conversationId}] Created tool call:`, JSON.stringify({
            id,
            name: toolCalls[toolCalls.length-1].function.name,
            arguments: toolCalls[toolCalls.length-1].function.arguments
          }));

          console.log(`[${conversationId}] Initialized tool call: id=${id}, name=${toolCalls[toolCalls.length-1].function.name}`);
        }
      } else {
        ...
        for (const tc of toolCallDeltas) {
          ...
          if (existingTool) {
            ...
            if (!tc.function) {
              console.warn(`[${conversationId}] Tool call delta missing function property for update: ${JSON.stringify(tc)}`);
              continue;
            }
            ...
            if (tc.function?.arguments) {
              ...
              console.log(`[${conversationId}] Updating arguments: existing="${existingTool.function.arguments || ''}", new="${tc.function.arguments}"`);
              ...
              try {
                ...
                if (concatenatedArgs.trim().startsWith('{') &&
                    (concatenatedArgs.match(/{/g) || []).length === (concatenatedArgs.match(/}/g) || []).length) {

                  // Try to parse it to validate
                  JSON.parse(concatenatedArgs);

                  // If we reach here, it's valid JSON - log success
                  console.log(`[${conversationId}] Successfully concatenated arguments into valid JSON`);
                }
              } catch (e) {
                // Log parsing failure but continue with concatenation
                console.log(`[${conversationId}] Concatenated arguments not yet valid JSON: ${e.message}`);
              }
              ...
              // Log the final state
              console.log(`[${conversationId}] Updated arguments: "${existingTool.function.arguments}"`);
            }
          }
        }
      }
  ...
  private validateToolCalls(toolCalls: any[], conversationId: string): any[] {
    return toolCalls
      .map((tc, index) => {
        ...
        if (!tc.function) {
          console.warn(`[${conversationId}] Skipping tool call without function property: ${JSON.stringify(tc)}`);
          return null;
        }
        ...
        if (!tc.function.name || tc.function.name.trim() === '') {
          ...
          console.log(`[${conversationId}] Applied fallback name for empty function name: ${tc.function.name}`);
        }
        ...
        if (typeof tc.function.arguments === 'string') {
          try {
            ...
            console.log(`[${conversationId}] Tool call arguments for ${tc.function.name} are valid JSON`);
          } catch (jsonError) {
            // If not valid JSON, apply advanced repairs
            console.warn(`[${conversationId}] Invalid JSON in tool call arguments for ${tc.function.name}: ${jsonError.message}`);
            ...
            try {
              ...
              const singleQuotesFixed = args.replace(/'([^']*)'(?=\s*:)/g, '"$1"').replace(/:\s*'([^']*)'/g, ': "$1"');
              ...
              if (openBrackets > closeBrackets) {
                ...
                console.log(`[${conversationId}] Added ${openBrackets - closeBrackets} closing brackets`);
              }
              ...
              try {
                ...
                console.log(`[${conversationId}] Successfully repaired JSON arguments for ${tc.function.name}`);
                ...
                try {
                  ...
                  if (args.includes(':')) {
                    ...
                    if (pairs && pairs.length > 0) {
                      ...
                      const rebuiltArgs = '{' + pairs.map(pair => {
                        const [key, value] = pair.split(':').map(p => p.trim());

                        // Ensure key is quoted
                        const formattedKey = key.startsWith('"') && key.endsWith('"') ?
                          key : `"${key.replace(/"/g, '')}"`;
                      ...
                      console.log(`[${conversationId}] Created completely rebuilt JSON: ${rebuiltArgs}`);
                    } else {
                  ...
                  console.error(`[${conversationId}] All JSON repair attempts failed for ${tc.function.name}, using empty object`);
                  tc.function.arguments = '{}';
                }
              }
            } catch (repairError) {
              // Something went wrong in our repair logic
              console.error(`[${conversationId}] Error in JSON repair process: ${repairError.message}`);
              tc.function.arguments = '{}';
            }
          }
        ...
        if (typeof tc.function.arguments === 'string') {
          try {
            ...
            console.error(`[${conversationId}] Final validation failed - forcing to empty object: ${finalValidationError.message}`);
            tc.function.arguments = '{}';
          }
        }
  ...
  private async processToolCalls(params: {
    ...
    try {
      ...
      console.log(`[${conversationId}] Starting tool calls processing with client:`,
