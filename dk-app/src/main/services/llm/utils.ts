...
export async function setupMCPConfig(provider: string = 'openai', options: any = {}) {
      ...
      if (mcpConfig.mcpServers.hasOwnProperty(name)) {
        ...
        try {
          ...
          if (!toolsResult || !toolsResult.tools || !Array.isArray(toolsResult.tools)) {
            console.error(
              chalk.red(`Error: Invalid tools result from ${name}: ${JSON.stringify(toolsResult)}`)
            )
            continue
          }
          ...
          const providerTools = mcpToolsToProviderTools(
            toolsResult.tools,
  ...
  if (mcpConfig && Object.keys(mcpConfig.mcpServers).length > 0) {
    console.log(
      chalk.green(`Connected to MCP servers: ${Object.keys(mcpConfig.mcpServers).join(', ')}`)
    )
  }
  ...
  for (const toolCall of toolCalls) {
    ...
    try {
        ...
        if (typeof toolCall.function.arguments === 'string') {
          ...
          if (argsStr === '') {
            ...
            try {
              ...
              console.warn(
                chalk.yellow(`JSON parse error, attempting to fix: ${parseError.message}`)
              )
              ...
              // Fix for arguments that are just a raw string without JSON formatting
                ...
                try {
                  ...
                  args = { value: argsStr }
                  console.log(
                    chalk.green(`Converted non-JSON string to object: ${JSON.stringify(args)}`)
                  )
                } catch (e) {
              ...
              // Fix for missing closing braces
                ...
                try {
                  ...
                  console.log(
                    chalk.green(
                      `Successfully fixed and parsed JSON arguments: ${JSON.stringify(args)}`
                    )
                  )
                } catch (fixError) {
          ...
          console.warn(
            chalk.yellow(
              `Arguments not string or object (${typeof toolCall.function.arguments}), using empty object`
            )
          )
          args = {}
        }
      ...
      if (!client.callTool) {
        console.error(
          chalk.red(
            `MCP client missing callTool method. Client keys: ${Object.keys(client).join(', ')}`
          )
        )
        ...
        const possibleMethods = Object.keys(client).filter(
          (k) => k.toLowerCase().includes('tool') || k.toLowerCase().includes('call')
        )
...
export function formatMessagesForProvider(messages: Message[], provider: ProviderType): any {
  if (provider === 'openai' || provider === 'openrouter') {
    console.log(chalk.cyan(`Formatting ${messages.length} messages for ${provider} provider`))
    ...
    return messages.map((msg) => {
      ...
      if (msg.tool_calls) {
        ...
        for (const tc of msg.tool_calls) {
          console.log(
            chalk.cyan(
              `Tool call: ${tc.function?.name || 'unnamed'}, args: ${tc.function?.arguments || '{}'}`
            )
          )
        }
...
export function processLLMResponse(
  ...
  try {
    if (provider === 'openai' || provider === 'openrouter') {
      ...
      if (!message) {
        console.error(
          `[${logId}] OpenAI response missing message:`,
